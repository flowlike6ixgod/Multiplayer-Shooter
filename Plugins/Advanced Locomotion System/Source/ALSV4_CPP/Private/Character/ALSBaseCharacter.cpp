...
FOnALSBaseCharacterEquipWeapon AALSBaseCharacter::NotifyEquipWeapon;
FOnALSBaseCharacterEquipWeapon AALSBaseCharacter::NotifyUnEquipWeapon;
	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UALSCharacterMovementComponent>(CharacterMovementComponentName))
{
	...
	bWantsToFire = false;
	WeaponAttachPoint = TEXT("VB LHS_ik_hand_gun");
}
...
void AALSBaseCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AALSBaseCharacter, Inventory, COND_OwnerOnly);
	
	DOREPLIFETIME(AALSBaseCharacter, CurrentWeapon);
}

void AALSBaseCharacter::Destroyed()
{
	Super::Destroyed();
	DestroyInventory();
}

void AALSBaseCharacter::PawnClientRestart()
{
...
void AALSBaseCharacter::BeginPlay()
{
	...
	{
		GetWorldTimerManager().SetTimerForNextTick(this, &AALSBaseCharacter::SpawnDefaulInventory);
	}
}
...
AWeaponBase* AALSBaseCharacter::GetCurrentWeapon() const
{
	return CurrentWeapon;
}

bool AALSBaseCharacter::bIsFiring()
{
	return bWantsToFire;
}
...
void AALSBaseCharacter::OnRep_CurrentWeapon(AWeaponBase* LastWeapon)
{
	SetCurrentWeapon(CurrentWeapon, LastWeapon);
}

void AALSBaseCharacter::AddWeapon(AWeaponBase* Weapon)
{
	if (Weapon && GetLocalRole() == ROLE_Authority)
	{
		Weapon->OnAddWeapon(this);
		Inventory.AddUnique(Weapon);
	}
}

void AALSBaseCharacter::RemoveWeapon(AWeaponBase* Weapon)
{
	if (Weapon && GetLocalRole() == ROLE_Authority)
	{
		Weapon->OnRemoveWeapon();
		Inventory.RemoveSingle(Weapon);
	}
}

AWeaponBase* AALSBaseCharacter::FindWeapon(TSubclassOf<AWeaponBase> WeaponClass)
{
...
void AALSBaseCharacter::EquipWeapon(AWeaponBase* Weapon)
{
	if (Weapon)
	{
		if (GetLocalRole() == ROLE_Authority)
		...
		{
			ServerSpawnInventory(Weapon);
			UE_LOG(LogTemp, Warning, TEXT("Else Equip Weapon"));
		}
	}
	UE_LOG(LogTemp, Warning, TEXT("Weapon == nullptr"));
}

FName AALSBaseCharacter::GetWeaponAttachPoint() const
{
	return WeaponAttachPoint;
}

int32 AALSBaseCharacter::GetInventoryCount() const
{
	return Inventory.Num();
}

AWeaponBase* AALSBaseCharacter::GetInventoryWeapon(int32 index) const
{
	return Inventory[index];
}

void AALSBaseCharacter::OnNextWeapon()
{
	if (Inventory.Num() >= 2 && (CurrentWeapon == nullptr || CurrentWeapon->GetCurrentWeaponState() != EWeaponState::Equipping))
	{
		const int32 CurrentIndex = Inventory.IndexOfByKey(CurrentWeapon);
		AWeaponBase* NextWeapon = Inventory[(CurrentIndex + 1) % Inventory.Num()];
		EquipWeapon(NextWeapon);
	}
}

void AALSBaseCharacter::OnPreviousWeapon()
{
	if (Inventory.Num() >= 2 && (CurrentWeapon == nullptr || CurrentWeapon->GetCurrentWeaponState() != EWeaponState::Equipping))
	{
		const int32 CurrentIndex = Inventory.IndexOfByKey(CurrentWeapon);
		AWeaponBase* PrevWeapon = Inventory[(CurrentIndex - 1 + Inventory.Num()) % Inventory.Num()];
		EquipWeapon(PrevWeapon);
	}
}

void AALSBaseCharacter::OnSwitchWeapon()
{

}

void AALSBaseCharacter::SpawnDefaulInventory()
{
	if (GetLocalRole() < ROLE_Authority)
	{
		return;
	...
	int32 NumClasses = DefaultInventoryClasses.Num();
...
void AALSBaseCharacter::DestroyInventory()
{
	if (GetLocalRole() < ROLE_Authority)
	{
		return;
...
void AALSBaseCharacter::ServerSpawnInventory_Implementation(AWeaponBase* Weapon)
{
	EquipWeapon(Weapon);
}

bool AALSBaseCharacter::ServerSpawnInventory_Validate(AWeaponBase* Weapon)
{
	return true;
}
