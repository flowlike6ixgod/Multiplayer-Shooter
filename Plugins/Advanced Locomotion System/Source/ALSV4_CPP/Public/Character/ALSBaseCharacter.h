...
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnALSBaseCharacterEquipWeapon, AALSBaseCharacter*, AWeaponBase*);
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnALSBaseCharacterUnEquipWeapon, AALSBaseCharacter*, AWeaponBase*);
...
public:
	...
	static FOnALSBaseCharacterEquipWeapon NotifyEquipWeapon;

	/** Global notification when a character un-equips a weapon. Needed for replication graph. */
	static FOnALSBaseCharacterUnEquipWeapon NotifyUnEquipWeapon;
...
protected:
	...
	float LookUpDownRate = 1.0f;

	UPROPERTY(EditDefaultsOnly, Category = "ALS|Input", BlueprintReadOnly)
	float LookLeftRightRate = 1.0f;
...
public:
	...
	/* Start firing from weapon (LOCAl) */
...
public:
	...
	/* Find weapon in character inventory() */
	AWeaponBase* FindWeapon(TSubclassOf<AWeaponBase> WeaponClass);
	...
	class AWeaponBase* GetInventoryWeapon(int32 index) const;
...
protected:
	/* Weapon attach socket name or VB*/
...
public:
	/* Spawn default inventory when spawned */
	void SpawnDefaulInventory();

	/* Remove inventory */
	void DestroyInventory();

	UFUNCTION(Reliable, Server, WithValidation)
	void ServerSpawnInventory(AWeaponBase* Weapon);
